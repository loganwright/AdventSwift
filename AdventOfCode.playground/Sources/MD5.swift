...
extension NSData {
    public func md5() -> NSData {
        ...
        var result = MD5(bytesArray).calculate()
...
func toUInt32Array(slice: ArraySlice<UInt8>) -> Array<UInt32> {
    ...
    for idx in slice.startIndex.stride(to: slice.endIndex, by: sizeof(UInt32)) {
        let one = UInt32(slice[idx.advancedBy(3)]) << 24
        let two = UInt32(slice[idx.advancedBy(2)]) << 16
        let three = UInt32(slice[idx.advancedBy(1)]) << 8
        let four = UInt32(slice[idx])
        let val: UInt32 = (one | two | three | four)
        result.append(val)
    }
    return result
}
...
final public class MD5 : HashProtocol  {
